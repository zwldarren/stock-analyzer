...
_realtime_cache_lock = threading.Lock()  # Thread lock for cache access
...
_etf_realtime_cache_lock = threading.Lock()  # Thread lock for ETF cache
...
class EfinanceFetcher(BaseFetcher):
    ...
    def get_realtime_quote(self, stock_code: str, **kwargs) -> UnifiedRealtimeQuote | None:
        ...
        try:
            # Check cache with thread lock to prevent race condition
            ...
            if df is None:
                # Trigger full refresh (outside lock to avoid blocking)
            ...
            quote = UnifiedRealtimeQuote(
                ...
                pb_ratio=safe_float(row.get(pb_col)),  # 市净率
